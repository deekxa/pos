...
import { 
  Minus, Plus, Trash2, Package, ShoppingCart, Grid3x3, List, Search, X, 
  Edit, Receipt, Users, UtensilsCrossed, Clock, TrendingUp, Tag
} from 'lucide-react'
...
export default function POSPage() {
  ...
  const updateTable = (tableId, tableNumber, capacity) => {
    const updatedTables = tables.map(t => 
      t.id === tableId 
        ? { ...t, number: tableNumber, capacity: capacity }
        : t
    )
    setTables(updatedTables)
    setTableModal({ show: false, mode: 'add', tableData: null })
    ...
    <ProtectedRoute allowedRoles={['admin', 'branch_head', 'cashier', 'worker']}>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {showTableView ? 'Select a table to start taking orders' : `Managing Table ${selectedTable?.number}`}
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">
                  रु{tables.reduce((sum, t) => sum + getTableTotal(t), 0).toLocaleString()}
                </p>
              </div>
        ...
        {showTableView ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-gray-900">Select a Table</h2>
              </div>
              <button
                onClick={() => setTableModal({ show: true, mode: 'add', tableData: null })}
                className="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors"
              >
                <Plus size={18} />
                Add Table
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {tables.map(table => (
                <div
                  key={table.id}
                  className={`relative border-2 rounded-xl p-6 transition-all ${getTableStatusColor(table.status)}`}
                >
                  <div onClick={() => selectTable(table)} className="text-center cursor-pointer">
                    <div className="text-3xl font-bold text-gray-900 mb-2">
                      {table.number}
                    </div>
                    <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${getTableStatusBadge(table.status)}`}>
                      {table.status}
                    </span>
          ...
          /* Order Management View - Same as before but keeping it for completeness */
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                    {filteredProducts.map(product => {
                      ...
                      return (
                        <div
                          key={product.id}
                          onClick={() => product.stock > 0 && addToTable(product)}
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="border-b border-gray-200 p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-lg font-bold text-gray-900 ${getTableStatusColor(selectedTable.status)}`}>
                        {selectedTable.number}
                      </div>
                      <div>
                        <h2 className="text-lg font-semibold text-gray-900">Table {selectedTable.number}</h2>
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${getTableStatusBadge(selectedTable.status)}`}>
                  ...
                  <div className="text-sm text-gray-500">
                    {selectedTable.orders.length} {selectedTable.orders.length === 1 ? 'item' : 'items'} • 
                ...
                <div className="p-5">
                  {selectedTable.orders.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400 text-sm mt-1">Add items from menu</p>
                    </div>
                  ) : (
                    <>
                          ...
                          <div key={item.id} className="border border-gray-200 rounded-lg p-3">
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-gray-600 text-sm">रु{item.price.toLocaleString()}</p>
                              </div>
                            ...
                            <div className="flex items-center justify-between mt-3">
                              <div className="flex items-center gap-2 border border-gray-200 rounded-lg">
                                <button
                                  onClick={() => updateTableQuantity(selectedTable.id, item.id, item.quantity - 1)}
                              ...
                              <button
                                onClick={() => removeFromTable(selectedTable.id, item.id)}
                                className="p-1.5 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
                              >
                                <Trash2 size={16} />
                              </button>
                      ...
                      <div className="border-t border-gray-200 pt-4 space-y-2">
                        <div className="flex justify-between text-sm text-gray-600">
                          <span>Subtotal</span>
                          <span className="font-medium">रु{getTableTotal(selectedTable).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-sm text-gray-600">
                          <span>Tax (10%)</span>
                          <span className="font-medium">रु{(getTableTotal(selectedTable) * 0.1).toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                          <span className="text-base font-semibold text-gray-900">Total</span>
                          <span className="text-xl font-bold text-gray-900">
                            रु{(getTableTotal(selectedTable) * 1.1).toLocaleString()}
                          </span>
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <form onSubmit={(e) => {
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setTableModal({ show: false, mode: 'add', tableData: null })}
      ...
      {reserveModal.show && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Reserve Table {tables.find(t => t.id === reserveModal.tableId)?.number}
            </h3>
            <form onSubmit={(e) => {
              e.preventDefault()
              const formData = new FormData(e.target)
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setReserveModal({ show: false, tableId: null })}
                  className="flex-1 px-4 py-2.5 bg-white text-gray-700 border border-gray-200 rounded-lg font-medium hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-sm w-full p-6">
            ...
            <p className="text-center text-gray-600 text-sm mb-6">
              This action cannot be undone. The {deleteModal.type} will be permanently removed.
            </p>
            <div className="flex gap-3">
              <button
